...
- Require complete source coverage and final enrichment before activation. Persist `ready` successfully before ordinary retrieval resumes; failed work remains resumable with its diagnostic reason. An AI-only transcript has no complete-turn boundary, so every message in the source range ends a batch turn
